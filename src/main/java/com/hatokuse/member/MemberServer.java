...
public class MemberServer {
    ...
    private static final String DEFAULT_LEADER_HOST = "127.0.0.1";
    ...
    private void connectToLeader() {
        // NettyChannelBuilder ile explicit InetSocketAddress kullan (Unix Domain Socket
        // yerine TCP)
        java.net.InetSocketAddress address = new java.net.InetSocketAddress(leaderHost, leaderPort);
        leaderChannel = io.grpc.netty.shaded.io.grpc.netty.NettyChannelBuilder.forAddress(address)
                .usePlaintext()
                .build();
    ...
    /**
     * Lidere kayıt olur (retry mekanizması ile).
     */
    private void registerWithLeader() {
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                RegisterRequest request = RegisterRequest.newBuilder()
                        ...
                        .setHost("127.0.0.1") // IPv4 adresi kullanılır
                        .setPort(port)
                        .build();
                ...
                System.err.println("[" + memberId + "] Lider iletişim hatası (deneme " + attempt + "/" + maxRetries
                        + "): " + e.getStatus().getCode());

                if (attempt < maxRetries) {
                    System.out
                            .println("[" + memberId + "] " + (retryDelay / 1000) + " saniye sonra tekrar denenecek...");
                    try {
        ...
        System.err.println("[" + memberId + "] Lidere kayıt olunamadı, maksimum deneme sayısına ulaşıldı.");
    }
